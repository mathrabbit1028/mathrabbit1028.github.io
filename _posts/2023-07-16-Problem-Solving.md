...
위 두 알고리즘 모두 장단점이 있다. 첫 번째 알고리즘은 $$|V|$$가 작은 경우에 더 빠르고, 두 번째 알고리즘은 $$M$$이 작은 경우에 더 빠르다. 이를 바탕으로 $$|V|$$와 $$M$$의 대소에 따라 알고리즘을 고르는 휴리스틱을 만들 수 있다.
...
이외에도 풀이는 다양하다. sqrt decomposition을 사용할 수도 있고, 각 $$|V|$$의 값을 최대한 균일하게 맞추는 방법 등등
