...
- 점화식
    - $$st$$번째 문자가 $$p_i$$의 $$k$$번째 문자와 같다면 계속 이어나가면 된다. 이를 점화식으로 표현하는 다음과 같다: 
    `dp[st][ed][i][k][full] <- dp[st + 1][ed][i][k + 1][full]`
        ...
        - 위의 약속에 따라 $$1≤i≤2C$$인 모든 $$i$$에 대해 다음 점화식이 성립한다: 
        `dp[st][ed][0][0][full] <- dp[st][ed][i][1][full]`
    - 시작점을 끊었더라도 중간 부분을 다른 사탕으로 팔고 다시 이어나갈수도 있다. 각 $$st<j\leq ed$$에 대하여 구간 $$[st, j)$$를 다른 사탕으로 파는 경우를 생각하면 다음 점화식을 얻는다: 
    ...
    - `full`의 값이 `false`이고 사탕이 끊어져 $$i$$와 $$k$$가 리셋된 경우에 한해 $$st$$번째 사탕을 넘길 수 있다. 이 경우 다음 점화식을 얻는다: 
